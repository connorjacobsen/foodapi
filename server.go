...
// The access token. In real-life scenarios, a more complex authentication
...
func init() {
	...
	r.Post(`/foods`, PostFood)
	r.Put(`/foods/:id`, PutFood)
...
// The regex to check for the requested format (allows optional trailing
// slash).
var rxExt = regexp.MustCompile(`(\.(?:xml|text|json))\/?$`)

// MapEncoder intercepts the request's URL, detectes the requested format,
...
func main() {
	go func() {
		...
		// APIs on separate web server from the web (html) pages, for maintenance and
		...
		// not transferred with every API request.
	...
	// Listen on https: with the preconfigure martini instance. The certificate files
